...
public class PatientDAO {
    ...
    /** إنشاء user(role=PATIENT) + patient في ترانزاكشن واحدة (باستخدام Connection خارجي). */
    public Patient createWithUser(Connection c,
                                  String fullName,
                                  String email,        // nullable
                                  String passwordHash, // BCrypt جاهز
    ...
    /** في حال عملت الـ user مسبقًا وتريد فقط إنشاء سجل patient */
    public Patient createForExistingUser(Connection c,
                                         long userId,
                                         LocalDate dob,
                                         Gender gender,
                                         String medicalHistory) throws SQLException {
        ...
        throw new SQLException("Failed to create patient for existing user");
    }
    ...
    /** بحث بسيط للواجهة (Autocomplete أو جدول) باسم المستخدم المرتبط */
    public List<Patient> searchByUserName(Connection c, String q, int limit, int offset) throws SQLException {
        final String sql = """
            SELECT p.*
            FROM patients p
            JOIN users u ON u.id = p.user_id
            WHERE u.full_name ILIKE ?
            ORDER BY similarity(u.full_name, ?) DESC, u.full_name
            LIMIT ? OFFSET ?
            """;
        try (PreparedStatement ps = c.prepareStatement(sql)) {
